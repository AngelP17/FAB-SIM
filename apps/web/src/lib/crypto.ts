...
export async function sha256Hex(parts: Uint8Array[]): Promise<`0x${string}`> {
...
export function stableStringify(obj: unknown): string {
  ...
  const sorter = (value: unknown): unknown => {
    if (value && typeof value === "object") {
      ...
      for (const k of Object.keys(value).sort()) out[k] = sorter((value as Record<string, unknown>)[k]);
      return out;
    }
    return value;
  };
...
export async function buildMerkleTree(leaves: Hex[]): Promise<{
  id: string;
  hash: Hex;
  left?: {
    id: string;
